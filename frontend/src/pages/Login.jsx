...
function Login() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isForgotPassword) {
        if (!otpSent) {
          const res = await axios.post(`${API_URL}/api/auth/forgot-password`, { email: trimmedEmail });
          setOtpSent(true);
          setError(res.data?.message || "OTP Sent! Please check your email.");
        } else {
          await axios.post(`${API_URL}/api/auth/reset-password`, { email: trimmedEmail, otp, newPassword: password });
        ...
        await axios.post(`${API_URL}/api/auth/register`, { name: trimmedName, email: trimmedEmail, password, role });
        setIsRegister(false);
        setError("Registration successful! Please login.");
      } else {
        const res = await axios.post(`${API_URL}/api/auth/login`, { email: trimmedEmail, password });
